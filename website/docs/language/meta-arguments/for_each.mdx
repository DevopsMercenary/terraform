...
For example, if you had a user resource that has a `for_each` element
...
You could refer to a specific user as follows

```hcl
resource "aws_iam_user_policy" "user_policy" {
  name = "user_policy"
  user = aws_iam_user.user["Alice"].name
}
```

Because `for_each` returns a map object, the traditional 'splat' syntax works a little differently. For Example, to get a list of all user name, the  `values` function can be used.

```hcl
resource "aws_iam_group_membership" "team" {
  name = "tf-testing-group-membership"
  user = values(aws_iam_user.user)[*].name
}
```
